...
# Marty the mountaineer focuses on monopolizing stone

def get_code():
    return 5

def get_aliases():
    return ['marty','marty the mountaineer',
...
def take_turn(computer,players,available_settlement_points,
    ...
    # Check current stone trade ratio
    stone_trade_ratio = 4
    if "any" in computer.ports or "?" in computer.ports:
        stone_trade_ratio = 3
    if "stone" in computer.ports:
        stone_trade_ratio = 2
...
def choose_settlement(computer,players,available_settlement_points):
    ...
    for tile in tiles:
        if tile.resource=="stone":
            stone_matrix.append(tile.index)

    # If an available point is on a stone tile, add it to the options
    settlement_options = []
    for element in stone_matrix:
        for point in available_settlement_points:
            if element in point.coordinate:
                settlement_options.append(point)

    # If the stone port is one of the available points, add it to the options
    for point in available_settlement_points:
        if point.is_port:
            if point.port_resource=="stone":
                settlement_options.append(point)

    # If there are no stone tiles open to play on, play on a non-stone tile
    ...
    #  stone port adds 8, plus an extra 2 for each stone tile it's on
    #  3:1 port adds 4
    best_positions = []
...
def choose_road(computer,players,available_roads):
    ...
    # If an available road has both points on a stone tile, add it to options
    road_options = []
    for element1 in stone_matrix:
        for element2 in stone_matrix:
            for rd in available_roads:
                if element1 in rd.point1.coordinate and \
                    element2 in rd.point2.coordinate:
                    road_options.append(rd)

    # If no roads have both points on a stone tile, find roads with just one
    #  point on a stone tile and add it to the options
...
def discard(computer,new_resource_count):
    ...
    while computer.resource_count()>new_resource_count:
        # Get rid of stone first, should be easy to get more!
        if computer.stone!=0:
            discarded_resource = "stone"
        # If there's no more stone, get rid of a random resource
        else:
            all_resources = []
...
def place_robber(computer,players,tiles,original_tile):
    ...
    # If there are stone tiles not occupied by computer, but by other players,
    #  prioritize those
    for tile in occupied_tiles:
        if tile.resource=="stone":
