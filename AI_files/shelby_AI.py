...
def get_aliases():
    return ['sheep','shelby','shelby the shepherd',
...
def take_turn(computer,available_settlement_points,available_roads,
    ...
    if (len(available_roads)!=0 or len(available_settlement_points)!=0) and \
        ...
        perform_trade(computer,"sheep","wood",app)
    ...
    if (len(available_roads)!=0 or len(available_settlement_points)!=0) and \
        ...
        perform_trade(computer,"sheep","brick",app)
    ...
    if len(available_settlement_points)!=0 and \
        ...
        perform_trade(computer,"sheep","wheat",app)
    ...
    if len(available_city_points)!=0 and \
        ...
        while computer.stone<3 and computer.sheep>=sheep_trade_ratio+1:
            perform_trade(computer,"sheep","stone",app)
...
def choose_settlement(computer,available_settlement_points,app):
    ...
    # Make a matrix of all the sheep tiles
    sheep_matrix = []
    for tile in tiles:
        if tile.resource=="sheep":
            sheep_matrix.append(tile.index)

    # If an available point is on a sheep tile, add it to the options
    settlement_options = []
    ...
    # If the sheep port is one of the available points, add it to the options
    for point in available_settlement_points:
        if point.is_port:
    ...
    #  Sheep port adds 8, plus an extra 2 for each sheep tile it's on
    #  3:1 port adds 4
    best_positions = []
...
def choose_road(computer,available_roads,app):
    ...
    # If an available road has both points on a sheep tile, add it to options
    road_options = []
    ...
    #  point on a sheep tile and add it to the options
